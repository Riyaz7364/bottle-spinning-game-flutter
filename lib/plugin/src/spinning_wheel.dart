...
class SpinningWheel extends StatefulWidget {
  ...
  const SpinningWheel({
    ...
    required this.initialSpinAngle,
    required this.spinResistance,
    this.canInteractWhileSpinning,
    this.stopSpin,
    this.secondaryImage,
    ...
    this.shouldStartOrStop,
       ...
       assert(
         secondaryImage == null ||
             (secondaryImageHeight != null && secondaryImageWidth != null),
       );

  @override
  State<SpinningWheel> createState() {
    // Avoid using private types in public APIs.
    return _SpinningWheelState();
  }
...
class _SpinningWheelState extends State<SpinningWheel>
      ...
      child: Stack(
        children: [
          widget.secondaryImage != null
              ...
              : Container(),
          GestureDetector(
            onPanUpdate: (widget.stopSpin != null && widget.stopSpin == true)
                ? null
                : _moveWheel,
            onPanEnd: (widget.stopSpin != null && widget.stopSpin == true)
                ? null
                : _startAnimationOnPanEnd,
            onPanDown: (details) =>
                (widget.stopSpin != null && widget.stopSpin == true)
                ? null
                : _stopAnimation(),
  ...
  void _updateAnimationValues() {
    if (_animationController.isAnimating) {
      ...
      _currentDistance = _motion.distance(
    ...
    _currentDivider =
        int.parse("${widget.dividers}") -
        (double.parse("$modulo") ~/ _dividerAngle);
  ...
  void _startAnimation(Offset pixelsPerSecond) {
    ...
    _animationController.duration = Duration(
      milliseconds: (_totalDuration * 1000).round(),
    );
